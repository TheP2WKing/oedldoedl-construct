...
{ 
    ...
    public static class ConfigLoder
    {
        public ConfigLoder(ForgeConfigSpec.Builder builder) 
        {
            ...
            lava_inactive = builder.comment("Toggle superheated lavas ability to destroy blocks. [Default: true]").define("lava_inactive", true);
            lava_fuel_duration = builder.comment("Set the fuel duration for superheated lava (in ticks). [Default: 40000]").defineInRange("lava_fuel_duration", 40000, 0, Integer.MAX_VALUE);
            builder.pop();
            ...
            enable_shard_drop = builder.comment("Toggle shard drop modifiers. [Default: true]").define("enable_shard_drop", true);
            builder.pop();
            
            enable_mod_recipes = builder.comment("Toggle all recipes from this mod that are considered misc. This means that only recipes that serve the purpose of obtaining that "
            		+ "item / block / liquid will be disabled. This option is intended for use in a modpack with custom recipes so you do not have to remove them with Crafttweaker. "
