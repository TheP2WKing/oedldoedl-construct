...
public class BlockFluidSadistium extends ModBlockFluidEffectBase {
	public final Potion effect2;
	public final Potion effect3;

	public BlockFluidSadistium(String modid, String name, Fluid fluid, Material material, MapColor mapColor,
			Potion effect, Potion effect2, Potion effect3, boolean isDebuff) {
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
		if (entityIn instanceof EntityLivingBase && ConstructConfig.PROPERTIES.EFFECTS.FLUIDS.FLUID_EFFECTS && ConstructConfig.PROPERTIES.EFFECTS.EFFECTS_EBALED) {
			((EntityLivingBase) entityIn).addPotionEffect(new PotionEffect(effect, ConstructConfig.PROPERTIES.EFFECTS.FLUIDS.FLUID_BASE_DURATION, ConstructConfig.PROPERTIES.EFFECTS.FLUIDS.FLUID_BASE_AMPLIFIER - 1 + 2, false, false));
			((EntityLivingBase) entityIn).addPotionEffect(new PotionEffect(effect2, ConstructConfig.PROPERTIES.EFFECTS.FLUIDS.FLUID_BASE_DURATION, ConstructConfig.PROPERTIES.EFFECTS.FLUIDS.FLUID_BASE_AMPLIFIER - 1 + 1, false, false));
			((EntityLivingBase) entityIn).addPotionEffect(new PotionEffect(effect3, 10, 0, false, false));
		}
	}
	...
	public void addProbeInfo(ProbeMode mode, IProbeInfo probeInfo, EntityPlayer player, World world,
		...
		if (player.isSneaking() && ConstructConfig.PROPERTIES.EFFECTS.FLUIDS.FLUID_EFFECTS && ConstructConfig.PROPERTIES.EFFECTS.EFFECTS_EBALED) {
			...
			ModTopTooltips.addPotionEffect(mode, probeInfo, player, world, blockState, data, effect3.getName(), isDebuff, getEffect3(), 20);
