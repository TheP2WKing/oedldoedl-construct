...
public class TraitGremoriusly extends AbstractTrait 
{
	...
	{
		return new TextComponentTranslation("trait.oedldoedlconstruct.gremoriusly.name").getFormattedText();
	}

	@Override
	public String getLocalizedDesc() 
	{
		return new TextComponentTranslation("trait.oedldoedlconstruct.gremoriusly.tip").getFormattedText();
	}
	...
	public void afterHit(ItemStack tool, EntityLivingBase player, EntityLivingBase target, float damageDealt, boolean wasCritical, boolean wasHit) 
	{
		...
		if (!target.isEntityAlive() && !world.isRemote) 
		{
        	...
        	{  	
        		items.add(drop(target, ItemInit.ITEM_GREMORIUM_NUGGET, MathHelper.getInt(rand, 0, 2), MathHelper.getInt(rand, 0, 2), MathHelper.getInt(rand, 0, 2)));
        	}
